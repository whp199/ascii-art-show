...
void plasma_draw(ScreenBuffer *buffer) {
    const char* charset = " .:-=+*#%@";
    ...
    for (int y = 0; y < buffer->height; y++) {
        for (int x = 0; x < buffer->width; x++) {
            ...
            int color_val = ((int)((val + 3.0) * 30.0) + color_shift) % 230 + 16;
             Color c = {(unsigned char)(sin(0.1 * color_val + 4) * 127 + 128),
                        (unsigned char)(sin(0.1 * color_val) * 127 + 128),
